...
COPY managed-schema.xml /opt/solr/server/solr/configsets/_default/conf/managed-schema.xml
COPY solr-kural-fields.json $CORE_FIELDS
COPY strapi-kural-sample.json $CORE_SAMPLE_DATA
COPY solr-extended-core.sh /opt/solr/docker/scripts/solr-extended-core.sh
USER $SOLR_UID

ENTRYPOINT ["solr-extended-core.sh"]